...
class Iamporter {
  ...
  /**
   * 여러 개의 포트원 거래고유번호 또는 고객사 주문번호로 결제내역을 한 번에 조회 (최대 100개)
   * GET - https://api.iamport.kr/payments?imp_uid%5B%5D={imp_uid}&merchant_uid%5B%5D={merchant_uid}
   * @see {@link https://api.iamport.kr/#!/payments}
   * 
   * @param {Array} impUids
   * @param {Array} merchantUids
   * @returns {Promise} result
   */
  findPaymentsByImpUidsOrMerchantUids(impUids, merchantUids) {
    let queryString = '';
    if (impUids && impUids.length > 0) {
      queryString += 'imp_uid%5B%5D=' + impUids.join('&imp_uid%5B%5D=');
    }
    if (merchantUids && merchantUids.length > 0) {
      if (queryString.length > 0) {
        queryString += '&';
      }
      queryString += 'merchant_uid%5B%5D=' + merchantUids.join('&merchant_uid%5B%5D=');
    }
  ...
  /**
   * 예약 거래주문번호(merchant_uid)로 결제예약정보 조회
   * GET - https://api.iamport.kr/subscribe/payments/schedule/{merchant_uid}
   * @see {@link https://api.iamport.kr/#!/subscribe/payments/schedule/merchant_uid}
   * 
   * @param {string} merchantUid
   * @returns {Promise} result
   */
   getScheduleSubscription(merchantUid) {
    const spec = {
      method: 'GET',
      url: `/subscribe/payments/schedule/${merchantUid}`,
    };
   ...
   * @see {@link https://api.iamport.kr/#!/vbanks/revokeVbank}
   * 
   * @param {string} impUid
   * @returns {Promise} result
   */
